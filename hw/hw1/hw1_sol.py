...
spawns = np.loadtxt("pogo_spawns_data.csv", delimiter=",") #Fill in the file name of your data
...
plt.scatter(spawns[:,1],spawns[:,0]) #FILL ME
...
data = np.loadtxt("mnist_images.csv", delimiter=",") #Fill in the file name of your data
...
img =  data[0,:784]#FILL ME

#Next reshape the image vector

img_reshaped = np.reshape(img, (28,28)) #FILL ME

#Finally we can visualize the image with plt's imshow function.

plt.imshow(img_reshaped) #FILL ME
plt.show()
...
nrow = img_reshaped.shape[0]
cimg = np.zeros((nrow-ksize+1,nrow-ksize+1))
xp = 0
yp = 0
for x in range(ksize-1,nrow-ksize+1):
	for y in range(ksize-1,nrow-ksize+1):
		cval = 0
		for a in range(ksize):
			for b in range(ksize):
				cval += img_reshaped[x - a,y - b] * kernel[a, b]
		cimg[xp,yp] = cval
		yp += 1
	yp = 0
	xp += 1
